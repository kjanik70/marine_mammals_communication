#!/usr/bin/env python3
"""Generate using 32K model with same prompts for comparison."""
...
def main():
    ...
    parser.add_argument("--prompts", nargs="+", default=["sanctsound_hi01_01_000365.npy", "sanctsound_hi05_01_000003.npy", "sanctsound_hi01_01_000366.npy"])
    args = parser.parse_args()

    token_dir = Path(args.token_dir)
    ckpt_path = Path(args.checkpoint)
    out_dir = Path(args.output_dir) if args.output_dir else ckpt_path.parent / "comparison_32k"
    ...
    for i, prompt_file in enumerate(args.prompts):
        ...
        # Save
        full_path = out_dir / f"full_{i:02d}_{stem}_32k.wav"
        sf.write(str(full_path), full_audio, sr)

        print(f"     balanced (T=0.85, top_k= 80)... gen={gen_dur:.1f}s → {full_path.name}")
